...
<body>
	...
	<!-- 변수 출력은 원래 c:out/ 인데 생략 가능 -->
	<!-- 예전에는 c:out이 필요했지만 요즘 버전은 생략해서 씀 -->
	숫자 1 : ${su1 }<br>
	숫자 2 : ${su2 }<br>
	
	숫자 1을 다른 방법으로 출력 : <c:out value="${su1 }"/><br>
	숫자 2을 다른 방법으로 출력 : <c:out value="${su2 }"/><br>
	
	<!-- if문 -->
	<!-- else가 없어서 반대 조건을 한번 더 써주면 됨 -->
	<!-- 한자리와 두자리 비교하니까 제대로 비교가안돼서 한자리앞에 0을 붙이거나 띄어쓰는걸로 해결함 -->
	<c:if test="${su1>su2 }">
		<h2>숫자 1이 더 크네요!!</h2>
	</c:if>
	<c:if test="${su1<su2 }"> 
	...
	<c:set var="today" value="<%=new Date() %>"/> <!-- 자바변수를 jstl 변수로 변환 -->
	<h3>급여 : ${money }</h3>
	<h3>오늘날짜 : ${today }</h3>
	
	<h3>숫자 (money,num)를 각종 양식으로 출력</h3>
	<!-- * 100 퍼센트로 출력 -->
